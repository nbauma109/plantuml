...
public class RibbonRectangle implements PDrawing {
	...
	public RibbonRectangle(TimingRuler ruler, ISkinParam skinParam, List<TimingNote> notes, boolean compact,
			TextBlock title, int suggestedHeight, Style style) {
		this.style = style;
	...
	private void drawPentaA(UGraphic ug, double len, ChangeState change) {
		...
		final PentaAShapeRectangle shape = PentaAShapeRectangle.create(len, getRibbonHeight(), context);
		shape.drawU(ug);
	}
